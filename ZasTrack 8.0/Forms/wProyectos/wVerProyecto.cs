...
namespace ZasTrack.Forms.wProyectos
{
    public partial class wVerProyecto : Form
    {
        ...
        private Panel? _panelSeleccionado = null;   
        private enum EstadoVista { Activos, Archivados }
        private EstadoVista vistaActual = EstadoVista.Activos;
        ...
        {
            if (sender is Button btnEditar && btnEditar.Parent?.Parent is Panel pnlContenedorTarjeta && pnlContenedorTarjeta.Tag is Proyecto proyectoParaEditar)
            {

                SeleccionarPanelProyecto(pnlContenedorTarjeta);
                ...
                {
                    DialogResult resultado = formEditar.ShowDialog(this);
                    if (resultado != DialogResult.Cancel)
                    {
                        Console.WriteLine("DEBUG: Editor cerrado. Recargando lista...");
                        CargarProyectosAsync();
                    }
                    else { Console.WriteLine("DEBUG: Editor cerrado con Cancelar."); }
                }
            }
        ...
        private void BtnDetalles_Click(object sender, EventArgs e)
        {
            ...
            {
                Console.WriteLine($"Abriendo detalles para ID: {proy.id_proyecto}, Nombre: {proy.nombre}");

                using (wVerDetallesProyecto formDetalles = new wVerDetallesProyecto(proy.id_proyecto))
                {
                    formDetalles.ShowDialog(this); // Mostrar como diálogo modal
                }
            }
            else
            {
        ...
        private void SeleccionarPanelProyecto(Panel pnlASeleccionar)
        {
            // Validar que el panel no sea null y tenga un Proyecto en el Tag
            if (pnlASeleccionar?.Tag is Proyecto proy)
            {
                // 1. Quitar resaltado del panel anteriormente seleccionado (si había uno)
                if (_panelSeleccionado != null && _panelSeleccionado != pnlASeleccionar)
                {
                    _panelSeleccionado.BackColor = SystemColors.Control; // O el color de fondo normal
                }

                // 2. Resaltar el panel recién clickeado
                pnlASeleccionar.BackColor = SystemColors.Highlight; // Color de resaltado

                // 3. Guardar referencias al panel y al proyecto seleccionado
                _panelSeleccionado = pnlASeleccionar;
                proyectoSeleccionado = proy; // La variable que ya usas

                Console.WriteLine($"Seleccionado (desde SeleccionarPanelProyecto): {proyectoSeleccionado.nombre}");

                // 4. Actualizar SIEMPRE el estado de los botones admin después de una selección
                ActualizarEstadoBotonesAdmin();
            }
            else
            {
                Console.WriteLine("WARN: SeleccionarPanelProyecto recibió un panel inválido o sin Tag de Proyecto.");
                // Opcional: Limpiar selección si el panel no es válido
                // if (_panelSeleccionado != null) _panelSeleccionado.BackColor = SystemColors.Control;
